!***********************************************************************
! Module to replace common blocks /woodssaxon/ and /nukrad/
!***********************************************************************
...
!     This is teh value stored in the common block /nukrad/
!
...
!  B. DECLARATIONS
!
    use nucdens_data_module, only: RR, a0, c0
    use nucdens_data_module, only: RAeq, RAws
    implicit none
...
!
    ...
    if ((Z .eq. 1) .and. (A .eq. 2)) then
        ...
        ! *** WOODS - SAXON
        if (firsttime(irho)) then
            if ((Z .eq. 0) .or. (Z .gt. 92) .or. (Rws(Z) .lt. 0.1d0)) then
...
!  B. DECLARATIONS
!
    use nucdens_data_module, only: RR => RR, a0 => a0, c0 => c0
    implicit none
...
!
    ...
    WoodsSaxon3d = 4*pi*r**2 * (1+c0*(r*r/(RR*RR))) / (1+dexp((r-RR)/a0))
    if (WoodsSaxon3d .lt. 0d0) WoodsSaxon3d = 0d0

    return
end function WoodsSaxon3d
...
!
    ...
    DATA W / 0.1012285362903762590D0, &
             0.2223810344533744710D0, &
             0.3137066458778872870D0, &
             0.3626837833783619830D0, &
             0.2715245941175409490D-1, &
             0.6225352393864789290D-1, &
             0.9515851168249278480D-1, &
             0.1246289712555338720D0, &
             0.1495959888165767320D0, &
             0.1691565193950025380D0, &
             0.1826034150449235890D0, &
             0.1894506104550684960D0/

    DATA X / 0.9602898564975362320D0, &
             0.7966664774136267400D0, &
             0.5255324099163289860D0, &
             0.1834346424956498050D0, &
             0.9894009349916499330D0, &
             0.9445750230732325760D0, &
             0.8656312023878317440D0, &
             0.7554044083550030340D0, &
             0.6178762444026437480D0, &
             0.4580167776572273860D0, &
             0.2816035507792589130D0, &
             0.9501250983763744020D-1/
!
!   ******************************************************************
!
!  START.
...
!
!  COMPUTATIONAL LOOP.
!  Outer loop: advance through intervals (replaces label 1)
    outer: do
        AA = BB
        BB = B
        ! Inner loop: subdivide until convergence (replaces label 2)
        inner: do
            C1 = 0.5D0*(BB+AA)
            C2 = 0.5D0*(BB-AA)
            S8 = 0.0D0
            ...
            END DO
            S16 = C2*S16
            IF (ABS(S16-S8) .LE. EPS*(1.+ABS(S16))) THEN
                ! Converged: accumulate result (was label 5)
                DGAUSS11 = DGAUSS11 + S16
                IF (BB .NE. B) THEN
                    CYCLE outer
                ELSE
                    RETURN
            ...
            END IF
            BB = C1
            IF (1.D0+ABS(CONST*C2) .NE. 1.D0) CYCLE inner
            ! Accuracy not achievable: error handling
            DGAUSS11 = 0.0D0
